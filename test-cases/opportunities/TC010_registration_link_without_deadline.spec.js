 ...
 * external navigation.
 *
 * Cost note: like TC009, running this creates one new "QA Test" Jobs listing each time.
 */
test('TC010 - Verify the registration link works without a deadline', async ({ page }) => {
  ...
  const title = `QA Test - TC010 automated ${Date.now()} (safe to delete)`;
  await createPage.titleInput.fill(title);
  await createPage.fillDescription('QA automated test listing for TC010. Safe to delete.');
